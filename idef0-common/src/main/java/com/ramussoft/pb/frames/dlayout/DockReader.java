...
class DockReader extends DockBoundary {
     ...
     */
    public DockReader(final int edge) {
        super(edge);
        ourLayoutReflects = edge == ToolBarLayout.SOUTH || edge == ToolBarLayout.EAST;
    }

    /**
     * Creates a ChainingDockBoundary for the specified edge with the provided
     * spacing.
     */
    public DockReader(final int edge, final int spacing) {
        super(edge, spacing);
        ourLayoutReflects = edge == ToolBarLayout.SOUTH || edge == ToolBarLayout.EAST;
    }
    ...
    public void validate() {
        ...
        for (final JToolBar toolbar : bars) {
            barLength = getPreferredToolBarLength(toolbar);

            if (totalBarLength != 0)
                totalBarLength += spacing;

            setToolBarBounds(toolbar, totalBarLength, 0, Math.min(barLength,
                    length), barDepth);
